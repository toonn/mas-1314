...
titles = { 'totalDistance' : 'Comparison of total distance traveled',
            ...
            'gTardiness' :
                'Lateness in minutes according to Gendreau obj. function',
            'gOverTime' :
...
for arg in ['Gallconfigurations_correct.json']:
    ...
    for var in ['gTardiness']:
        for conf_i, config in enumerate(
                ['bestfuture simple hcommr hrui',
                    'bestfuture simple hcommr mrui',
                    'bestfuture simple hcommr lrui',
                    'bestfuture simple mcommr hrui',
                    'bestfuture simple defaults',
                    'bestfuture simple mcommr lrui',
                    'bestfuture simple lcommr hrui',
                    'bestfuture simple lcommr mrui',
                    'bestfuture simple lcommr lrui']):
        ...
        plt.subplots_adjust(top=.95, bottom=.18, left=.1, right=.99)
        plt.legend(loc='upper center', bbox_to_anchor=(0.5, -0.13),
                ncol=4, prop={'size' : 6})
        
        plt.savefig('plots/' + 'latex_commR_Lateness' + '.pdf')
        plt.clf()
