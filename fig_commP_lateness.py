...
for arg in ['Gallconfigurations_correct.json']:
    ...
    for var in ['gTardiness']:
        for conf_i, config in enumerate(
                ['bestfuture simple httl',
                    'bestfuture simple defaults',
                    'bestfuture simple lttl',
                    'bestfuture simple lcommrel']):
        ...
        plt.subplots_adjust(top=.95, bottom=.15, left=.1, right=.99)
        plt.legend(loc='upper center', bbox_to_anchor=(0.5, -0.13),
                ncol=4, prop={'size' : 6})
        
        plt.savefig('plots/' + 'latex_commP_Lateness' + '.pdf')
        plt.clf()
