	...
	public void renderDynamic(GC gc, ViewPort vp, long time) {
		...
		synchronized (objects) {
			for (final SmartVehicle a : objects) {
				...
				final int x = (int) (vp.origin.x + (p.x - vp.rect.min.x)
						* vp.scale);
				final int y = (int) (vp.origin.y + (p.y - vp.rect.min.y)
						* vp.scale);
				...
				} else if (a.getReliability() >= 0.3
						&& a.getReliability() < 0.6) {
				...
				gc.drawText("r:" + a.getNoReceived(), x, y + 4 * size, true);

				final Set<SmartVehicle> communicatedWith = a
						.getCommunicatedWith();
				for (final SmartVehicle cw : communicatedWith) {
					...
					final int xCW = (int) (vp.origin.x + (p.x - vp.rect.min.x)
							* vp.scale);
					final int yCW = (int) (vp.origin.y + (p.y - vp.rect.min.y)
							* vp.scale);
